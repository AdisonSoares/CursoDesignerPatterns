 package aula06.parte06_Singleton_Aplicacao_EagerSingleton_MultiThread_Solucao_;
 ...
 * 
 * @MecanismoTrava_Lock
 * Para auxiliar na cordenação dos metodos executando em diferentes threads, 
 * dessa forma a trava garante que ao entrar uma thread no método essa trava
 * é concedida e outra thread só entra quando a thread anterior sair.
 * 
 ...
 * pela primeira vez.
 */
public class EagerSingleton_MultiThread_Solucao {
	//Método Eager para atribuir o primeiro objeto dentro da instancia
	private static EagerSingleton_MultiThread_Solucao instancia = new EagerSingleton_MultiThread_Solucao();
	
	private EagerSingleton_MultiThread_Solucao() {
	}
	
	public static EagerSingleton_MultiThread_Solucao getInstancia() {
		/* NÃO PRECISA MAIS DO TESTE POIS JÁ FOI INICIALIZADO O OBJETO E SYNCHRONIZED FOI TIRADO
		if(instancia == null) {
			instancia = new EagerSingleton_MultiThread_Solucao();
		}
		*/		
		return instancia;
	}
